        ...
        try:
            ...
            with open(file_path, "r", encoding="utf-8") as f:
                ...
                for line in f:
                    ...
                    new_x = (
                        float(re.search(r"X([-+]?\d*\.?\d+)", line).group(1))
                        if "X" in line
                        else cur_x
                    )

                    new_y = (
                        float(re.search(r"Y([-+]?\d*\.?\d+)", line).group(1))
                        if "Y" in line
                        else cur_y
                    )
            ...
            self.finalize_plot(
                f"Gコード解析完了 : {os.path.basename(file_path)}",
                found_large_r,
            )

        except Exception as e:

            self.show_error(f"Gコードエラー : {str(e)}")
